...
/*   Updated: 2018/11/07 20:21:40 by abiri            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*parse3(char **str)
{
	if (**str)
	{
		if (cins("sSpdDioOuUxXcC", **str))
			return (ft_strsub((*str)++, 0, 1));
		if (cins("hl", **str))
		{
			if (**str == *(*str + 1))
			{
				if (ft_strchr("dDioOuUxXpcCsS", *(*str + 2)) && ((*str += 3)))
					return (ft_strsub(*str - 3, 0, 3));
			}
			if (ft_strchr("dDioOuUxXpcCsS", *(*str + 1)) && ((*str) += 2))
				return (ft_strsub(*str - 2, 0, 2));
		}
		if (cins("zj", **str))
		{
			if (cins("dDioOuUxXsScC", *(*str + 1)) && ((*str) += 2))
				return (ft_strsub(*str - 2, 0, 2));
		}
	}
	while (cins("hljz", **str))
